...
Follow the established coding standards from CLAUDE.md. This repo is Python (LangGraph graph +
FastAPI backend) plus TypeScript (Next.js frontend):

**Python (graph + backend):**
- PEP 8; type hints on public functions and node signatures
- Pydantic v2 models for structured data (policy, offers, comparison, recommendation)
- Keep deterministic nodes as pure functions; keep LLM-call nodes thin (build prompt → call → parse)
- Do date/deadline math in plain Python, never via the LLM

**TypeScript (Next.js frontend):**
- Explicit prop types; named exports; colocate component-local types with the component
- Consistent import ordering

Defer to `CLAUDE.md` as the source of truth when it and this list disagree.
